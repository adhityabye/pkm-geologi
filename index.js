class StoneViewer {
  constructor(stoneData) {
    ...
    this.featureContainer = document.querySelector(
  ...
  setStone(index) {
    ...
    selectedStone.features.forEach((feature) => {
      ...
      newFeatureButton.addEventListener("click", (event) => {
        // if there's another overlay displayed, just hide
        this.hideOverlay();
  ...
  prev() {
    this.selectedIndex =
      (this.selectedIndex - 1 + this.stoneData.length) % this.stoneData.length;
    this.setStone(this.selectedIndex);
  }
  ...
  toggleInfoVisibility() {
    ...
    if (this.isInfoVisible) {
      ...
      this.viewerContainer.style.height = "50%";
      this.infoToggle.textContent = "Hide Info";
    } else {
      this.infoContainer.classList.add("hidden");
      this.viewerContainer.style.height = "100%";
      this.infoToggle.textContent = "Show Info";
    }
  }
